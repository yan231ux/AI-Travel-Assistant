...
4. rag_guide(destination)
   检索本地攻略知识库（支持：北京/上海/成都/重庆/杭州/丽江/三亚/大理/厦门/西安）。
...
- 攻略数据在 `../zhilv-yuntu-main/backend/data/*.md`（原 Python 版 6 城；Java 版已扩展到 10 城）
- 已知城市：北京、上海、成都、重庆、杭州、丽江、三亚、大理、厦门、西安
...
known-cities: 北京,上海,成都,重庆,杭州,丽江,三亚,大理,厦门,西安
```
...
> 迁移完成后追加的优化， worth noting for defense.

### 13.1 数据可信度治理（v1.1）

| 问题 | 关键修复 |
|---|---|
| 景点地址/描述张冠李戴 | 攻略结构化成卡片（位置/简介/门票/时长），生成后按景点名匹配并强制回写 |
| 餐厅凭空编造 | 餐食必须从 POI 餐厅池选取，缺失时自动替换 |
| 酒店混进景点列表 | 同名酒店从 spots 中移除，酒店只能写入 `hotel` 字段 |
| 预算编造 | `source_notes` 禁止核算金额，金额由后端 `calculateBudget` 汇总 |
| 指定景点被静默忽略 | 从 `special_notes` 提取点名景点，定向查询 + 提示词硬保证；做不到时结果中明确标注 |

### 13.2 性能：三级缓存（v1.1）

1. **外部数据缓存**（Redis）：高德 POI/路线 24h、天气 30min、RAG 6h；
2. **THINK 计划缓存**（内存 1h）：同城市 + 同偏好复用搜索计划；
3. **生成结果缓存**（Redis 30min）：同参数 + 同用户复用完整行程。

> 关键：结果/计划缓存 key 含 `userId`，防止用户间串号。

### 13.3 长期记忆 / 个性化（v1.2）

- `UserProfileService` 基于当前用户历史 `trip_record` 做确定性统计画像（去过的城市、节奏、住宿档次、餐饮口味、平均预算等）；
- 生成前把画像注入 prompt【用户历史记忆】段；
- 结果 `source_notes` 标注“已结合你的历史行程定制”；
- `userId` 必须在主线程从 `UserContext`（ThreadLocal）捕获后再传入异步线程，异步线程中读不到 ThreadLocal。

### 13.4 前端日历选择器（v1.1）

`Home.vue` 日期输入由纯文本改为 `a-date-picker` 日历选择器：禁用过去日期、结束日期 ≥ 开始日期、最长 7 天、三联动。
